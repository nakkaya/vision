...
#+begin_src clojure :eval query :tangle no
  ...
  (let [frame (load-image image-file :color)
    ...
    (doseq [c team]
      (circle frame c java.awt.Color/red))
    
    (doseq [c pink]
      (circle frame c java.awt.Color/yellow))
    
    (plus frame (id-10 team pink) java.awt.Color/red)
    (plus frame (id-09 team pink) java.awt.Color/red)
...
#+begin_src clojure :eval no :tangle no
  (defn circle [[pointer image] [x y r] c]
    (let [g (.getGraphics image)]
      (.setColor g c)
      (.draw g (java.awt.geom.Ellipse2D$Double. (- x r) (- y r) (* 2 r) (* 2 r)))))
#+end_src

#+srcname: clojure-graphics-plus
#+begin_src clojure :eval no :tangle no
  (defn plus [[pointer image] [x y] c]
    (let [g (.getGraphics image)]
      (.setColor g c)
